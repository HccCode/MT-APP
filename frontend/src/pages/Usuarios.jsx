...
export default function Usuarios({ token, usuario, esAdmin, estructuraGeografica, handleLogout }) {
  ...
  const cancelarEdicionUser = () => { setIdUserEditando(null); setNewUsername(''); setNewPassword(''); setNewNombreCompleto(''); setNewRole('RNOC'); setNewPlazas(['*']); setNewNumEmpleado(''); setNewCorreo(''); setNewArea(''); setNewRegionUsuario(''); setNewPuesto(''); setMsgUser(''); };
  ...
  const handleProcesarUsuario = async (e) => {
    ...
    try {
      const res = await fetch(url, { method: idUserEditando ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ username: newUsername, password: newPassword, role: newRole, plazas: plazasString, nombre_completo: newNombreCompleto, num_empleado: newNumEmpleado, correo: newCorreo, area_org: newArea, region_asignacion: newRegionUsuario, puesto: newPuesto }) });
      if (res.status === 401) { handleLogout(); return; }
  ...
  const handleActivarModoEdicionUser = (u) => { setIdUserEditando(u.id); setNewUsername(u.username || ''); setNewRole(u.role || 'RNOC'); setNewPassword(''); setNewNombreCompleto(u.nombre_completo || ''); setNewPlazas(u.plazas ? u.plazas.split(',') : []); setNewNumEmpleado(u.num_empleado || ''); setNewCorreo(u.correo || ''); setNewArea(u.area_org || ''); setNewRegionUsuario(u.region_asignacion || ''); setNewPuesto(u.puesto || ''); };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-1 bg-[#0b132b]/50 border border-slate-800 rounded-xl p-5 space-y-4">
          <form onSubmit={handleProcesarUsuario} className="space-y-3" autoComplete="off">
            ...
            <div className="grid grid-cols-2 gap-3"><div><label className="text-[10px] text-slate-500 block mb-1">No. Empleado *</label><input type="text" value={newNumEmpleado} onChange={e => setNewNumEmpleado(e.target.value)} required className="w-full bg-[#1c2541] border border-slate-700 text-xs p-2 rounded text-white focus:outline-none focus:border-blue-500" /></div><div><label className="text-[10px] text-slate-500 block mb-1">Rol Sistema *</label><select value={newRole} onChange={e => setNewRole(e.target.value)} required className="w-full bg-[#1c2541] border border-slate-700 text-xs p-2 rounded text-white focus:outline-none focus:border-blue-500"><option value="">Seleccione...</option><option value="RNOC">RNOC (Solo Lectura - Inv)</option><option value="MCM NOC">MCM NOC (Lectura/Escritura - Inv)</option><option value="MCM INGENIERIA">MCM INGENIERIA (L/E Inv + Carga Excel)</option><option value="ADMIN">ADMIN (Superusuario)</option></select></div></div>
            <div><label className="text-[10px] text-slate-500 block mb-1">Correo Electrónico *</label><input type="email" value={newCorreo} onChange={e => setNewCorreo(e.target.value)} required className="w-full bg-[#1c2541] border border-slate-700 text-xs p-2 rounded text-white focus:outline-none focus:border-blue-500" /></div>
            <div className="grid grid-cols-2 gap-3"><div><label className="text-[10px] text-slate-500 block mb-1">Área Org. *</label><input type="text" value={newArea} onChange={e => setNewArea(e.target.value)} required className="w-full bg-[#1c2541] border border-slate-700 text-xs p-2 rounded text-white focus:outline-none focus:border-blue-500" /></div><div><label className="text-[10px] text-slate-500 block mb-1">Puesto *</label><input type="text" value={newPuesto} onChange={e => setNewPuesto(e.target.value)} required className="w-full bg-[#1c2541] border border-slate-700 text-xs p-2 rounded text-white focus:outline-none focus:border-blue-500" /></div></div>
        ...
        <div className="md:col-span-3 bg-[#0b132b]/30 border border-slate-800 rounded-xl p-5 flex flex-col space-y-4">
          ...
          <div className="overflow-x-auto"><div className="inline-block min-w-full overflow-hidden align-middle"><div className="overflow-y-auto max-h-[500px] border border-slate-800 rounded-lg custom-scrollbar"><table className="min-w-full text-xs text-left whitespace-nowrap"><thead className="bg-[#0b132b] text-slate-500 border-b border-slate-800 sticky top-0 z-10"><tr><th className="p-3 font-bold">Username</th><th className="p-3 font-bold">Role</th><th className="p-3 font-bold">No. Empleado</th><th className="p-3 font-bold">Área / Puesto</th><th className="p-3 font-bold">Visibilidad Ciudades</th><th className="p-3 font-bold text-right sticky right-0 bg-[#0b132b]">Acciones</th></tr></thead><tbody className="divide-y divide-slate-800/50">{usuariosFiltrados.map(u => { const uRole = String(u.role).trim().toUpperCase(); const isUserAdmin = u.username.toLowerCase() === 'admin' || uRole === 'ADMIN'; return (<tr key={u.id} className="hover:bg-slate-900/30"><td className="p-3 font-medium text-slate-200"><div className="font-bold text-white">{u.nombre_completo || 'Sin nombre'}</div><div className="text-[10px] text-slate-400">@{u.username}</div>{u.correo && <div className="text-[9px] text-slate-500 mt-0.5">{u.correo}</div>}</td><td className="p-3"><span className={`px-2 py-0.5 rounded text-[10px] ${isUserAdmin ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : uRole === 'MCM INGENIERIA' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : uRole === 'MCM NOC' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'bg-slate-500/10 text-slate-400 border border-slate-500/20'}`}>{uRole}</span></td><td className="p-3 text-slate-400 font-mono">{u.num_empleado || '-'}</td><td className="p-3 text-slate-300">{u.area_org || '-'}{u.puesto && <div className="text-[10px] text-slate-500 mt-0.5">{u.puesto}</div>}</td><td className="p-3 text-[10px] text-slate-400"><div className="max-w-[150px] truncate" title={obtenerNombresPlazas(u.plazas)}>{obtenerNombresPlazas(u.plazas)}</div></td><td className="p-3 text-right sticky right-0 bg-[#070b19] md:bg-transparent md:backdrop-blur-sm"><div className="flex justify-end gap-1.5"><button type="button" onClick={() => handleActivarModoEdicionUser(u)} title="Editar Usuario" className="p-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded transition-colors cursor-pointer"><Edit className="w-5 h-5" /></button><button type="button" onClick={() => handleEliminarUsuario(u.id, u.username)} disabled={usuario?.username === u.username} title="Eliminar Usuario" className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded transition-colors cursor-pointer disabled:opacity-20"><Trash2 className="w-5 h-5" /></button></div></td></tr>)})} {usuariosFiltrados.length === 0 && (<tr><td colSpan="6" className="p-6 text-center text-slate-500 italic">No se encontraron usuarios.</td></tr>)}</tbody></table></div></div></div>
        </div>
      </div>
    </main>
  );

...
export default function Cuadrilla({ token, handleLogout }) {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100dvh] z-[9999] bg-[#050814] flex flex-col overflow-hidden">
      ...
      <div className="bg-[#0b132b] border-b border-slate-800 px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] flex justify-between items-center shrink-0 shadow-md relative z-20">
        <div className="flex items-center gap-3">
            <h1 className="text-slate-100 font-black text-[15px] tracking-widest flex items-center gap-1 leading-none">MT<span className="text-indigo-500">_MANAGER</span></h1>
            <span className="bg-blue-900/40 text-blue-400 text-[8px] font-black px-2 py-0.5 rounded border border-blue-800 flex items-center gap-1 w-max"><ShieldAlert className="w-2.5 h-2.5" /> LECTURA</span>
        </div>
        <button onClick={cerrarSesion} className="flex items-center gap-1.5 text-[9px] uppercase font-black tracking-widest text-slate-300 bg-slate-800/80 px-2.5 py-1.5 rounded-lg border border-slate-700 active:bg-slate-700 active:scale-95 transition-all shadow-sm"><LogOut className="w-3 h-3 text-red-400" /> Salir</button>
      </div>
      ...
      <div className={`flex flex-col h-full w-full max-w-md mx-auto p-3 transition-transform duration-300 ${puertoActivo ? '-translate-x-full absolute opacity-0' : 'translate-x-0'}`}>
        ...
        {/* TÍTULO Y SELECTORES COMPACTOS */}
        <div className="flex flex-col gap-2 mb-3 shrink-0">
            <h2 className="text-lg font-black text-indigo-400 text-center mb-1">Trabajo en Campo</h2>
            
            {/* 1. SELECCIÓN DE TECNOLOGÍA */}
            <div className="flex bg-[#050814] p-1 rounded-lg border border-slate-800 shadow-inner">
                <button
                  onClick={() => { setPestanaActiva('FO'); setBusqueda(''); setResultadosFO([]); setResultadosMW([]); }}
                  className={`flex-1 py-1.5 px-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all flex justify-center items-center gap-1.5 ${pestanaActiva === 'FO' ? 'bg-[#4f46e5] text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  <Server className="w-3.5 h-3.5" /> F. Óptica
                </button>
                <button
                  onClick={() => { setPestanaActiva('MW'); setBusqueda(''); setResultadosFO([]); setResultadosMW([]); }}
                  className={`flex-1 py-1.5 px-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all flex justify-center items-center gap-1.5 ${pestanaActiva === 'MW' ? 'bg-[#4f46e5] text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  <Wifi className="w-3.5 h-3.5" /> Microondas
                </button>
            </div>

            {/* 2. TABS DE CRITERIO (SOLO FO) */}
            {pestanaActiva === 'FO' && (
              <div className="flex gap-1 bg-[#050814] p-1 rounded-lg border border-slate-800">
                <button onClick={() => { setCriterioBusqueda('CLIENTE'); setBusqueda(''); }} className={`flex-1 text-[10px] font-black uppercase py-1.5 rounded-md transition-colors ${criterioBusqueda === 'CLIENTE' ? 'bg-indigo-600/30 text-indigo-300 border border-indigo-500/50 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Cliente / ID</button>
                <button onClick={() => { setCriterioBusqueda('RUTA'); setBusqueda(''); }} className={`flex-1 text-[10px] font-black uppercase py-1.5 rounded-md transition-colors ${criterioBusqueda === 'RUTA' ? 'bg-indigo-600/30 text-indigo-300 border border-indigo-500/50 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Ruta FO</button>
              </div>
            )}
        </div>

        {/* 3. BUSCADOR */}
        <form onSubmit={(e) => { e.preventDefault(); ejecutarBusqueda(busqueda, criterioBusqueda); }} className="relative mb-3 shrink-0">
          <input 
            type="text" 
            placeholder={criterioBusqueda === 'RUTA' ? "Ej. RT20..." : "Buscar nombre, IP o puerto..."} 
            value={busqueda}
            onChange={(e) => setBusqueda(e.target.value)}
            className="w-full bg-[#0b132b] text-white text-sm py-3 pl-10 pr-10 rounded-xl border border-indigo-500/50 shadow-[0_0_10px_rgba(99,102,241,0.1)] outline-none focus:border-indigo-400 transition-colors"
          />
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-400 pointer-events-none" />
          
          {busqueda.length > 0 && (
            <button type="button" onClick={() => { setBusqueda(''); setResultadosFO([]); setResultadosMW([]); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition-colors">
              <X className="w-4 h-4" />
            </button>
          )}
        ...
        {!cargando && resultadosFO.length === 0 && resultadosMW.length === 0 && busquedasRecientes.length > 0 && busqueda.length === 0 && (
          <div className="mb-4 animate-in fade-in shrink-0">
            <div className="flex items-center justify-center gap-1.5 mb-2.5">
              ...
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Búsquedas Recientes</p>
            </div>
        ...
        {cargando && <p className="text-center text-indigo-400 animate-pulse font-bold flex justify-center items-center gap-2 mt-2 text-sm"><Activity className="w-4 h-4"/> Consultando MT_DB...</p>}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2.5 pb-8">
          ...
          {resultadosActuales.length === 0 && !cargando && busqueda.length > 2 && (resultadosFO.length > 0 || resultadosMW.length > 0) && (
             <p className="text-center text-slate-500 text-xs italic mt-4">No hay resultados con los filtros actuales.</p>
          )}

          {resultadosFO.length === 0 && resultadosMW.length === 0 && !cargando && busqueda.length > 2 && (
            <p className="text-center text-slate-500 text-xs italic mt-4">No se encontraron coincidencias.</p>
          )}
          ...
          {resultadosActuales.map((p, idx) => {
            ...
            return (
              <div 
                key={p.ID || p.id || idx} 
                className="bg-[#0b132b] border border-slate-700 p-3.5 rounded-xl shadow-lg cursor-pointer active:scale-95 transition-transform relative overflow-hidden animate-in fade-in slide-in-from-bottom-2" 
              ...
              >
                ...
                <p className="text-xs text-indigo-300 font-bold mb-2 truncate">
                  ...
                  <>
                    {criterioBusqueda === 'CLIENTE' ? (
                      ...
                      (rutaNombre || bufferVal || hiloVal) && (
                        <div className="mb-2.5 p-2 bg-[#050814]/80 rounded-lg border border-slate-800 flex flex-wrap items-center justify-between gap-1.5 text-xs">
                          {rutaNombre && (
                          ...
                          <div className="flex items-center gap-1.5 ml-auto">
                            {bufferVal && (
                              <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-black uppercase shadow-sm flex items-center gap-1 ${obtenerColorFibra(bufferVal)}`}>
                                <Layers className="w-2 h-2 opacity-70" /> {bufferVal}
                              </span>
                            )}
                            {hiloVal && (
                              <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-black uppercase shadow-sm flex items-center gap-1 ${obtenerColorFibra(hiloVal)}`}>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3.5 space-y-2.5 pb-10">
              ...
              {puertoActivo._tipo === 'FO' && (
                <>
                  <div className="bg-[#0b132b] border border-slate-800 rounded-xl p-3 shadow-sm">
                    <h3 className="text-white font-black text-base truncate mb-1">{puertoActivo.SERVICIO || 'Sin Cliente'}</h3>
              ...
              {puertoActivo._tipo === 'MW' && (
                <>
                  <div className="bg-[#0b132b] border border-slate-800 rounded-xl p-3 shadow-sm">
                    <h3 className="text-white font-black text-base truncate mb-1">{puertoActivo.cliente || 'Sin Cliente'}</h3>
